      ...
      <div className="flex justify-center items-center relative" style={{ width: '100%', maxWidth: '900px', height: '400px' }}>
        {dealtCards.map((card, index) => {
          ...
          return (
            ...
            >
              <div
                className={`w-48 h-64 lg:w-56 lg:h-80 ${card.image ? '' : card.bg} ${card.borderColor} border-4 rounded-xl shadow-2xl drop-shadow-lg overflow-hidden ${card.image ? '' : 'flex items-center justify-center'} transition-all duration-300 group-hover:scale-105 group-hover:-translate-y-6 cursor-pointer`}
                style={{
              ...
              >
                {card.image ? (
                  <img src={card.image} alt={card.name} className="w-full h-full object-cover rounded-lg" style={{ imageRendering: 'crisp-edges' }} />
                ) : (
